...
float TIME_DURATION = 15.0f; // Duration of the other example (seconds)
...
/*****************************
 * Controllers, please store your controller function in here
 * copy paste the example controller back into 02-low-level_full_example.cpp*
 *****************************/
...
bool example_cyclic_torque_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, k_api::ActuatorConfig::ActuatorConfigClient* actuator_config)
{
    ...
    try
    {
        ...
        float torque_amplification = 6.0f;
        //changed the above amplification from 2.0 to 6.0f as per youtube example video CHARS
...
int main(int argc, char **argv) {
    ...
    // Example core this runs the low_level_velocity control, and then the torque_control example.
    //bool success = example_actuator_low_level_velocity_control(base, base_cyclic);
    //success &= example_cyclic_torque_control(base, base_cyclic, actuator_config);
    bool success = controller(base, base_cyclic, actuator_config);
    if (!success) {
        std::cout << "There has been an unexpected error in example_cyclic_armbase() function." << endl;
    }
        ...
        while (timer_count < (TIME_DURATION * 1000))
        {
            ...
            if (now - last > 1000)
            {
                ...
                *****************************/
                ...
                //kd and kp values
                Kp[0] = 5; //gives up to 21 with 360 degree error
                Kd[0] = 0.7;
